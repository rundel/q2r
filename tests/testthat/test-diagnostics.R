...
test_that("pampa_parse_ts raises an error by default on bad input", {
  expect_error(pampa_parse_ts(bad_qmd), "Parse error")
})

test_that("pampa_parse_pd(quiet = TRUE) returns silently with diagnostics attached", {
  res = expect_no_condition(pampa_parse_pd(bad_qmd, quiet = TRUE))
...
test_that("pampa_parse_ts(quiet = TRUE) returns silently with diagnostics attached", {
  res = expect_no_condition(pampa_parse_ts(bad_qmd, quiet = TRUE))
...
test_that("clean input does not signal a condition under either default", {
  expect_no_condition(pampa_parse_pd("# hi"))
  expect_no_condition(pampa_parse_ts("# hi"))
  expect_no_condition(pampa_parse_pd("# hi", quiet = TRUE))
  expect_no_condition(pampa_parse_ts("# hi", quiet = TRUE))
})

test_that("pampa_parse_pd error message includes the rendered diagnostic text", {
  err = tryCatch(pampa_parse_pd(bad_qmd), error = identity)
...
test_that("pampa_parse_pd() coalesces distinct parser errors into a single stop()", {
  res = pampa_parse_pd(multi_err_qmd, quiet = TRUE)
  ...
  caught = catch_signals(pampa_parse_pd(multi_err_qmd))
  expect_s3_class(caught$error, "error")
  msg = conditionMessage(caught$error)
  for (t in titles) expect_match(msg, t, fixed = TRUE)
})

test_that("pampa_parse_pd(quiet = TRUE) preserves all diagnostics when there are several", {
  res1 = pampa_parse_pd(multi_err_qmd, quiet = TRUE)
  res2 = pampa_parse_pd(multi_err_qmd, quiet = TRUE)
...
test_that("prune_errors = TRUE (default) deduplicates parser-error diagnostics", {
  pruned   = pampa_parse_pd(bad_qmd, quiet = TRUE)
  unpruned = pampa_parse_pd(bad_qmd, quiet = TRUE, prune_errors = FALSE)
...
test_that("prune_errors = FALSE preserves raw diagnostics for ts parser too", {
  pruned   = pampa_parse_ts(bad_qmd, quiet = TRUE)
  unpruned = pampa_parse_ts(bad_qmd, quiet = TRUE, prune_errors = FALSE)
  expect_lt(length(pruned@diagnostics), length(unpruned@diagnostics))
})

test_that("prune_errors keeps distinct errors at separate ERROR nodes", {
  res = pampa_parse_pd(multi_err_qmd, quiet = TRUE, prune_errors = TRUE)
