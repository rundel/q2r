...
- [R/from-rust.R](R/from-rust.R) and [R/to-rust.R](R/to-rust.R) convert between the tagged-list shape that crosses the extendr boundary and the S7 `pandoc` hierarchy. `pandoc_from_list` / `pandoc_to_list` are the top-level entry points used by `pampa_parse_pd()` and `to_qmd(pandoc)` respectively.
...
- Current pinned commit: `99e7f89cf75fcdae406914080a209b016f656290`. Bumps are deliberate: update the `rev` **and** regenerate `Cargo.lock`. All four q2 deps (`pampa`, `tree-sitter-qmd`, `quarto-source-map`, `quarto-error-reporting`) must be bumped together.
