...
- [src/rust/src/ast_to_r.rs](src/rust/src/ast_to_r.rs) and [src/rust/src/cst_to_r.rs](src/rust/src/cst_to_r.rs) convert the Pandoc AST and tree-sitter CST into tagged nested lists. Note: CST `text` semantics diverge from AST: CST leaves always carry their source text, and any non-leaf whose children do not cover its full byte range (a grammar "gap": `pandoc_math`, `pandoc_display_math`, `code_fence_content`, and structural leads like the `](` in `target`) also carries its full source span. This is required for [`to_qmd()`](R/cst-to-qmd.R) to reconstruct bytes that tree-sitter-qmd parses via anonymous regexes and therefore never surfaces as named nodes. Revisit if the upstream grammar ever promotes those bytes to real CST nodes.
...
## CST → QMD rendering contract

- `to_qmd()` is lossy by design: it normalizes whitespace rather than preserving it byte-for-byte. Re-parsing the output should produce a structurally identical CST (same kind tree), but the source bytes may differ (blank line counts, trailing spaces, indentation).
- Per-kind handlers live in `ts_kind_handlers` in [R/cst-to-qmd.R](R/cst-to-qmd.R) and are derived empirically + by consulting [grammar.js](../q2/crates/tree-sitter-qmd/tree-sitter-markdown/grammar.js). Extending to a new kind: run a few parses, observe how that kind's children map to source bytes, and add a handler.
- `@text` on non-leaves is the escape hatch for grammar gaps. Handlers that use it: `pandoc_math`, `pandoc_display_math`, `code_fence_content`. If upstream tree-sitter-qmd promotes those anonymous regexes to named nodes, this fallback (and the corresponding Rust exporter logic in [cst_to_r.rs](src/rust/src/cst_to_r.rs)) can be removed.
