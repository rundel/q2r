...
- [R/ts-ast-to-qmd.R](R/ts-ast-to-qmd.R) defines `to_qmd()` (S7 generic on `ts_tree` / `ts_node`), which walks the tree-sitter AST and emits QMD source text via a per-kind handler table (`ts_kind_handlers`). The goal is **functional equivalence**: the output must re-parse to a structurally identical `ts_ast`. Byte-level differences from the input are only acceptable where the grammar treats them as non-significant (e.g., collapsed blank-line runs, trimmed trailing whitespace, an appended trailing newline). For nodes whose children leave gaps (math, `code_fence_content`) the handlers fall back to `@text` populated by the Rust exporter. Unknown kinds emit a warning and default to plain child concatenation. Use [`ts_to_text()`](R/ts-ast.R) when byte-exact reconstruction is required instead.
...
- `to_qmd()` aims for **functional equivalence**, not byte-for-byte reproduction: the output must re-parse to a structurally identical `ts_ast` (same kind tree, same node attributes). Any byte-level difference from the input is a bug unless it is demonstrably non-significant to the grammar (collapsed blank-line runs, trimmed trailing whitespace, an appended final newline). If in doubt, round-trip through `pampa_parse_ts()` and diff the kind tree — that's the correctness check.
